...
var sub1_completed := false   # ✅ Nuevo: bandera de completado


func _ready() -> void:
	print("🔍 Buscando botones dentro de Sub1Controller...")
	...
	var good_sign = $"Sub1_HintSign2"
	var bad_sign = $"Sub1_HintSign1"
	...
	print("✅ Sub1Controller inicializó los HintSigns como interactivos.")
...
func _on_good_sign_demo() -> void:
	...
	_show_hint_sequence(step_good.sequence, $"Sub1_HintSign2")

func _on_bad_sign_demo() -> void:
	print("👁️ HINT: Mostrando secuencia mala (con sonido).")
	_show_hint_sequence(step_bad.sequence, $"Sub1_HintSign1")
...
func _on_button_kicked(button: Node) -> void:
	...
	if _cursor >= current_sequence.size():
		if mode == Mode.GOOD:
			await _handle_sequence_completed($"Sub1_HintSign2", "GOOD")
		else:
			await _handle_sequence_completed($"Sub1_HintSign1", "BAD")
		_reset()
		return
...
func _handle_sequence_completed(sign: Node, label: String) -> void:
	...
	if sub1_completed and label == "BAD":
	...
	match label:
		"GOOD":
			sub1_completed = true   # ✅ Marcamos subnivel como completado
			# 🔑 Activar llave física
			var key1 := $"../../Interactables/Keys/Key1"
			if key1:
				key1.visible = true
				var shape := key1.get_node_or_null("CollisionShape2D")
				if shape:
					shape.disabled = false
				print("🔑 Key1 activada en el mundo. El jugador puede recogerla ahora.")
			else:
				print("🚨 No se encontró Key1 en ../../Interactables/Keys/")
